...
CREATE TABLE IF NOT EXISTS `laravel`.`stock` (
  ...
  `flag` INT NULL,
  `user_stock_id` INT UNSIGNED NOT NULL,
  PRIMARY KEY (`id`, `user_stock_id`))
ENGINE = InnoDB;
...
WHERE m.flag = '1' 
AND m.id NOT IN (SELECT b.material_id 
				 FROM bring_material_detail AS b
				 WHERE b.flag='1'
				 );

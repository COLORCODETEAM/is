...
CREATE TABLE IF NOT EXISTS `laravel`.`users` (
  ...
  `flag` INT NULL,
  `user_role_id` INT UNSIGNED NOT NULL DEFAULT 3,
...
CREATE TABLE IF NOT EXISTS `laravel`.`mapping_computer_detail` (
  ...
  PRIMARY KEY (`id`, `device_id`, `mapping_computer_id`),
  INDEX `fk_mapping_computer_device1_idx` (`device_id` ASC),
  UNIQUE INDEX `update_user_UNIQUE` (`update_user` ASC),
  INDEX `fk_mapping_computer_detail_mapping_computer1_idx` (`mapping_computer_id` ASC),
  CONSTRAINT `fk_mapping_computer_device1`
...
-- -----------------------------------------------------
-- Placeholder table for view `laravel`.`view_sumMappingComputerDetail`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `laravel`.`view_sumMappingComputerDetail` (`device_id` INT, `sum` INT);
...
d.serial_no,
d.warranty,
d.amount-(IFNULL(l.sum,0)+IFNULL(rb.sum,0)+IFNULL(rd.sum,0)+IFNULL(m.sum,0)) AS amount,
...
-- -----------------------------------------------------
-- View `laravel`.`view_sumMappingComputerDetail`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `laravel`.`view_sumMappingComputerDetail`;
USE `laravel`;
CREATE  OR REPLACE VIEW `view_sumMappingComputerDetail` AS
SELECT m.device_id, 1 AS sum
FROM mapping_computer_detail AS m 
LEFT JOIN mapping_computer AS mp ON m.mapping_computer_id=mp.id
WHERE m.flag='1' 
